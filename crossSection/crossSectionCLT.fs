 ...
 */
export function computeSectionBounds(sectionPoints is array) returns map
{
...
function assembleSectionMechanics(section is map, bodies is array) returns map
{
    ...
    for (var bodyInfo in section.bodyData)
    {
        ...
        // Skip bodies without material data
        ...
        var yBar_k = props.centroid2D[0];       // height above base (frame X = thickness)
        var Ixx_centroid_k = props.Iyy;         // second moment about width axis

        // Skip bodies with effectively zero area
        ...
        // Per-body linear mass density: mass per unit length
    ...
    // Guard against zero extensional stiffness
    if (abs(A[0][0]) > 1e-10 * newton)
    {
        // Neutral axis: the Y location where axial strain is zero under pure bending
        neutralAxisY = -B[0][0] / A[0][0];

        // Effective bending stiffness: accounts for extension-bending coupling
        // EI_eff = D11 - B11² / A11
