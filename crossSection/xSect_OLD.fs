...
export const eiXSect = defineFeature(function(context is Context, id is Id, definition is map)
    ...
    }
    {
        ...
        var namePrefix = definition.analysisName;
        
        // --- Create EI curve ---
        println("Creating EI curve with " ~ size(crossSectionData.crossSections) ~ " sections");
        try
        {
            createEICurve(context, id + "eiCurve", crossSectionData, namePrefix);
            println("EI curve created successfully");
        }
        catch
        {
            println("WARNING: createEICurve failed");
        }
        ...
        try
        {
            ...
            println("Neutral axis curve created successfully");
        }
        catch
        {
            println("WARNING: createNeutralAxisCurve failed");
        }
        
        // --- Beam stiffness analysis ---
        var stiffness = undefined;
        var fcpX = undefined;
        var acpX = undefined;
        try { fcpX = resolveReferencePointX(context, definition.fcpQuery, definition.xSectAlong); }
        catch {}
        try { acpX = resolveReferencePointX(context, definition.acpQuery, definition.xSectAlong); }
        catch {}
        ...
        if (fcpX != undefined && acpX != undefined)
        {
            ...
            stiffness = computeBeamStiffness(eiData, xFCP, xACP);
        ...
        // --- Total mass: sum of (volume * density) for all bodies with material ---
        var totalMass = computeTotalMass(crossSectionData.bodies);
        println("Total mass = " ~ totalMass);
        
        // --- Set attributes on Origin ---
        // Wrapped in try/catch: attribute failure must NOT roll back curves/composites
        try
        {
            setAnalysisAttributes(context, id, crossSectionData, stiffness, totalMass,
                                  namePrefix, definition.cleanOriginData);
        }
        catch
        {
            println("WARNING: setAnalysisAttributes failed");
        }
...
/**
 * Compute total mass from all bodies with material data.
 * Mass = volume * density for each body.
 *
 * @param bodies {array} : Top-level bodies array with volume and materialData
 * @returns {ValueWithUnits} : Total mass in kg
 */
function computeTotalMass(bodies is array) returns ValueWithUnits
{
    ...
    for (var body in bodies)
    {
        ...
        {
            var matData = tryGetKey(body, "materialData");
            if (matData != undefined)
            {
                var density = matData.density;
                var volume = tryGetKey(body, "volume");
                if (volume != undefined)
                {
...
/**
 * ORIGIN ATTRIBUTE: "EIData"
 * ==========================
 *
 * Single attribute on Origin containing ALL EI analyses, keyed by feature ID.
 *
 * Structure:
 * {
 *     "_keys" : ["id1", "id2", ...],          // ordered list of analysis keys
 *     "id1" : {
 *         "analysisName" : string,             // user-facing label
 *         "summary" : [                        // stiffness table rows
 *             { "metric" : string, "value" : number or string }, ...
 *         ],
 *         "sections" : [                       // section table rows
 *             {
 *                 "x" : number (mm, 2 sf),
 *                 "origin" : string "(x, y, z)",
 *                 "normal" : string "(nx, ny, nz)",
 *                 "EI" : number (N*m2, 1 sf),
 *                 "na" : number (mm, 2 sf),
 *                 "ld" : number (kg/m, 3 sf),
 *                 "width" : number (mm, 2 sf),
 *                 "height" : number (mm, 2 sf)
 *             }, ...
 *         ]
 *     },
 *     "id2" : { ... }
 ...
 *
 * Serialization rules:
 *   - Only: string, number, boolean, and arrays/maps thereof
 *   - NO: Vector3D, CoordSystem, ValueWithUnits, undefined, Query
 *
 * The custom table reads this attribute and produces a TableArray with
 * a summary + section table pair for every key in "_keys".
 */

// Attribute name constant
const EI_DATA_ATTR = "EIData";

/**
 * Read-modify-write the EIData attribute on Origin.
 *
 * @param context {Context}
 * @param id {Id} : Feature ID (first element used as map key)
 * @param crossSectionData {map} : CLT-processed cross-section data
 * @param stiffness : Beam stiffness results (or undefined)
 * @param totalMass {ValueWithUnits} : Total mass in kg
 * @param analysisName {string} : User-facing label
 * @param cleanOriginData {boolean} : If true, wipe all other analyses
 */
function setAnalysisAttributes(context is Context, id is Id, crossSectionData is map,
                                stiffness, totalMass is ValueWithUnits,
                                analysisName is string, cleanOriginData is boolean)
{
    var originEntity = { "queryType" : QueryType.TRANSIENT, "transientId" : "IB" } as Query;
    
    // Feature ID as string key
    var idKey = "" ~ id[0];
    println("setAnalysisAttributes: idKey=" ~ idKey ~ " analysisName=" ~ analysisName);
    
    // Build this feature's display-ready data
    var thisData = buildTableData(crossSectionData, stiffness, totalMass, analysisName);
    println("buildTableData completed: " ~ size(thisData.summary) ~ " summary rows, " ~
            size(thisData.sections) ~ " section rows");
    
    // Read existing attribute (or start fresh)
    var existingKeys = [];
    var existingData = {};
    
    if (cleanOriginData == false)
    {
        try
        {
            var existing = getAttribute(context, { "entity" : originEntity, "name" : EI_DATA_ATTR });
            if (existing != undefined)
            {
                existingData = existing;
                var ek = tryGetKey(existing, "_keys");
                if (ek != undefined)
                {
                    existingKeys = ek;
                }
            }
            println("Read existing EIData: " ~ size(existingKeys) ~ " keys");
        }
        catch
        {
            println("No existing EIData attribute (first run)");
        }
    }
    else
    {
        println("cleanOriginData=true, starting fresh");
    }
    ...
    if (found == false)
    {
        newKeys = append(newKeys, idKey);
    }
    
    // Build fresh output map (avoid mutating existing map)
    var output = {};
    output["_keys"] = newKeys;
    
    // Copy existing entries (skip current key — we'll overwrite it)
    for (var k in existingKeys)
    {
        if (k != idKey)
        {
            var old = tryGetKey(existingData, k);
            if (old != undefined)
            {
                output[k] = old;
            }
    ...
    // Add this feature's data
    output[idKey] = thisData;
    
    println("Writing EIData: " ~ size(newKeys) ~ " analyses");
    ...
    println("EIData attribute set successfully");
}

/**
 * Build display-ready data for one analysis.
 * All values are plain numbers or strings -- no ValueWithUnits, no Vectors.
 */
function buildTableData(crossSectionData is map, stiffness,
                         totalMass is ValueWithUnits, analysisName is string) returns map
{
    // --- Summary rows (stiffness table) ---
    var summary = [];
    
    if (stiffness != undefined)
    {
        summary = append(summary, { "metric" : "Prismatic stiffness (lb/in)",
                                     "value" : sigFigs(stiffness.prismaticStiffness_lbin, 3) });
        summary = append(summary, { "metric" : "Prismatic deflection (mm @ 30kg)",
                                     "value" : sigFigs(stiffness.prismaticStiffness_mm, 3) });
        summary = append(summary, { "metric" : "Estimated stiffness (lb/in)",
                                     "value" : sigFigs(stiffness.estimatedStiffness_lbin, 3) });
        summary = append(summary, { "metric" : "Estimated deflection (mm @ 30kg)",
                                     "value" : sigFigs(stiffness.estimatedStiffness_mm, 3) });
    }
    else
    {
        summary = append(summary, { "metric" : "Prismatic stiffness (lb/in)", "value" : "-" });
        summary = append(summary, { "metric" : "Prismatic deflection (mm @ 30kg)", "value" : "-" });
        summary = append(summary, { "metric" : "Estimated stiffness (lb/in)", "value" : "-" });
        summary = append(summary, { "metric" : "Estimated deflection (mm @ 30kg)", "value" : "-" });
    }
    
    // Mass in grams, 1 decimal place
    var massG = (totalMass / kilogram) * 1000;
    summary = append(summary, { "metric" : "Total mass (g)",
                                 "value" : floor(massG * 10 + 0.5) / 10 });
    ...
    for (var i = 0; i < size(crossSectionData.crossSections); i += 1)
    {
        ...
        // Sum lineal density
        var totalLD = 0 * kilogram / meter;
        for (var contrib in mp.bodyContributions)
        {
            totalLD += contrib.linearDensity;
        }
        
        // Strip units
        var x_mm = frame.origin[0] / millimeter;
        var naY_mm = mp.neutralAxisY / millimeter;
        var ei_Nm2 = mp.EI_eff / (newton * meter * meter);
        var ld_kgm = totalLD / (kilogram / meter);
        var w_mm = mp.sectionWidth / millimeter;
        var h_mm = mp.sectionHeight / millimeter;
        
        // Format origin string "(x, y, z)"
        var ox = sigFigs(frame.origin[0] / millimeter, 2);
        var oy = sigFigs(frame.origin[1] / millimeter, 2);
        var oz = sigFigs(frame.origin[2] / millimeter, 2);
        var originStr = "(" ~ ox ~ ", " ~ oy ~ ", " ~ oz ~ ")";
        
        // Format normal string "(nx, ny, nz)"
        var nx = sigFigs(frame.zAxis[0], 3);
        var ny = sigFigs(frame.zAxis[1], 3);
        var nz = sigFigs(frame.zAxis[2], 3);
        var normalStr = "(" ~ nx ~ ", " ~ ny ~ ", " ~ nz ~ ")";
        ...
        sections = append(sections, {
            ...
            "na" : sigFigs(naY_mm, 2),
            "ld" : sigFigs(ld_kgm, 3),
            "width" : sigFigs(w_mm, 2),
            "height" : sigFigs(h_mm, 2)
        });
    }
    
    // Label: use analysisName if provided, otherwise just "Analysis"
    var label = "Analysis";
    if (analysisName != "")
    {
        label = analysisName;
...
/**
 * Round a dimensionless number to n significant figures.
 *
 * Find the magnitude (largest power of 10 <= |val|), scale so the
 * significant digits sit in integer range, round, scale back.
 *
 * Examples:
 *   sigFigs(346.2, 1)  -> 300
 *   sigFigs(9.14, 1)   -> 9
 *   sigFigs(6.53, 2)   -> 6.5
 *   sigFigs(0.0047, 2) -> 0.0047
 *   sigFigs(1234, 2)   -> 1200
 */
function sigFigs(val, n is number) returns number
{
    if (abs(val) < 1e-15)
        return 0;
    ...
    // Find magnitude: largest power of 10 <= v
    var mag = 1.0;
    if (v >= 1)
    {
        while (mag * 10 <= v)
    ...
    // factor scales v so that n sig figs land in integer range
    var factor = 1.0;
    for (var i = 0; i < n - 1; i += 1)
        factor *= 10;
    factor = factor / mag;
    
    return sign * floor(v * factor + 0.5) / factor;
}
