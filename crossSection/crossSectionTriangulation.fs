 ...
 * ===================================
 *
 * Converts intersection curves to triangular meshes with section properties.
 *
 * Key capabilities:
 * - Groups curves into closed boundaries
 * - Detects nesting (holes within perimeters)
 ...
 *
 * @param bodyCurves {array} : Array of { BSplineCurve, ... }
...
export function earClipTriangulate(points2D is array, pointIndices is array) returns array
{
    ...
    while (size(localIndices) > 3)
    {
        ...
        if (!earFound)
        {
            // Degenerate case: check if remaining vertices are collinear
            ...
            if (allCollinear)
            {
                // Fan triangulate remaining vertices
            ...
            {
                println("ERROR: Triangulation stuck with " ~ numLocal ~ " non-collinear vertices");
                break;
            }
        }
    }
...
function addSectionProperties(a is map, b is map) returns map
{
    ...
    if (abs(totalArea) > 1e-20 * meter * meter)
    {
        ...
        // Weighted average of 3D centroids
